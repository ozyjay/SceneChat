...
**Hardware gate:** exact gateway image request and combined-load measurements in `MODEL_COMPATIBILITY.md`. Request shape, route/capability readiness, concurrency, reset/privacy invalidation, timeout, explicit fallback and outage degradation have offline coverage. Historical July 2026 evidence recorded a runtime-package-0.2.2, 140-visual-token candidate in `Open2026` revision 35 under an operator exception; that revision must not be assumed active. Current acceptance must discover and record the published route first, begin with one fixed image, then use only a short attended camera check with memory and temperature monitoring. Sustained benchmarking remains separately gated.
