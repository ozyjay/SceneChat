...
- `vision/` isolates deterministic offline providers and the ModelDeck gateway behind one provider protocol. The live adapter discovers the published `scenechat-vision` route through `/v1/routes`, verifies its `scene-analysis-v1` contract and readiness, then checks `image_input` and `structured_output` through `/v1/capabilities`. It does not require native scene routes in `/v1/models`. It accepts only JPEG/PNG input and adds trusted operational metadata only after strict output validation. ModelDeck may schedule its own Workers on private ports; SceneChat never addresses them directly.
