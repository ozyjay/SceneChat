...
4. Send ten approved image requests through the ModelDeck gateway; record median/p95 latency and failure count and confirm every request targets port `8600`. With SceneChat and ModelDeck already running, stop the camera, disable automatic analysis, select the `modeldeck` provider, then run `env SCENECHAT_MODELDECK_ACCEPTANCE=1 .venv/bin/python -m pytest tests/hardware/test_modeldeck_acceptance.py -v -s`. The test uses only the prepared replay image, performs two warm-ups followed by ten measured requests, reports ModelDeck round-trip and SceneChat end-to-end latency without printing descriptions, and fails above an 8-second median or 12-second p95.
